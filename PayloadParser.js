...
function getTrackerData(decodedData)
{
    ...
    for (let tag in decodedData) {
        ...
        switch (tag){
            ...
            case "valid":
                break;
            case "messages":
                var tagvalues = decodedData[tag];
                tagvalues.forEach(valueElement => {
                    valueElement.forEach(element => {
                       decoded.timeStamp = new Date(element.timestamp).toUTCString();
                       switch (element.type){
                        case "Longitude":
                        ...
                        case "Firmware Version":
                            decoded.firmwareVersion = element.measurementValue;
                            break;
                        case "Hardware Version":
                            decoded.hardwareVersion = element.measurementValue;
                            break;
                        case "Wi-Fi Scan":
                        case "Work Mode":
                        ...
                        case "Event Status":
                            break;
                        default:
                            env.log("Error: Tipo de mensaje desconocido: ", element.type);
                            break;
                        }
                    });
                });    
                break;
            default:
                env.log("Error: Atributo desconocido: ", tag);
                break;
        }
    }
    return decoded;
...
function parseUplink(device, payload)
{
    ...
    var decodedData = Decoder (bytes, payload.port)
    var trackerData = getTrackerData(decodedData.data);
    env.log(decodedData);
    env.log('------------------------------------------');
    env.log(trackerData);
